...
function calculadoraTotal() {
    ...
    const valor    = document.getElementById('preview-total');
    ...
    const recalc = () => {
        ...
        if (unidad) {
            ...
            unidad.textContent = par ? (n === 1 ? par[0] : par[1]) : 'periodos';
        }

        if (total > 0) {
            valor.textContent = fmtMoney(total);
            box.classList.add('is-active');
            const par = UNIDADES[freq.value];
            const uni = par ? (n === 1 ? par[0] : par[1]) : 'periodos';
            hint.textContent = `${fmtMoney(c)} × ${n} ${uni}`;
        } else {
            valor.textContent = fmtMoney(0);
            box.classList.remove('is-active');
            hint.textContent = 'Captura la cuota y el tiempo para ver el total.';
        }
    };
